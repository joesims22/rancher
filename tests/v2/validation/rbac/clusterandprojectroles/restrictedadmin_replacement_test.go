...
func (ra *RestrictedAdminReplacementTestSuite) TestRestrictedAdminReplacementCreateCluster() {
	...
	_, restrictedAdminClient, err := rbac.SetupUser(ra.client, rbac.RaReplacement.String())
	require.NoError(ra.T(), err)
	ra.T().Logf("Validating restricted admin replacement can create a downstream cluster")
